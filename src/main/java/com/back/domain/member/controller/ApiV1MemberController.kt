...
//    private val response: HttpServletResponse
    ...
    fun join(@RequestBody @Valid reqBody: @Valid MemberJoinReqBody): RsData<MemberJoinResBody> {
        ...
        val member: Member= memberService.join(reqBody.username, reqBody.password, reqBody.nickname)
    ...
    fun login(@RequestBody @Valid reqBody: @Valid MemberLoginReqBody): RsData<MemberLoginResBody> {
        ...
        val accessToken: String = memberService.genAccessToken(actor);
    ...
    fun me(): MemberDto {
        ...
        val tmpActor: Member  = rq.actor // user1 정보

        // 내 전체 회원 정보 조회가 목적
        val realActor: Member = memberService.findById(tmpActor.id).getOrThrow();
