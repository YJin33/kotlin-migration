...
//    private val memberService: MemberService,
    ...
    fun list(): List<PostDto> {
        val result: List<Post> = postService.findAll()

        val postDtoList: List<PostDto> = result.reversed()
                .map { PostDto(it) }
    ...
    fun detail(@PathVariable id: Int): PostDto {
        ...
        val post: Post = postService.findById(id).getOrThrow();
        return PostDto(post);
    }
    ...
    data class PostWriteReqBody(
            ...
            val content:  @NotBlank(message = "02-content-내용은 필수입니다.")
    ...
    fun write(
        ...
        val actor: Member= rq.actor // 인증된 사용자 정보 가져오기

        val post: Post= postService.write(actor, reqBody.title, reqBody.content)
